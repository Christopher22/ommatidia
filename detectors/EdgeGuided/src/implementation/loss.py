...
def get_seg2ptLoss(op, gtPts, temperature=1):
    ...
    if str(op.device) == "cpu":
        ...
        xloc = XYgrid[0, :, :, 0].reshape(-1).cpu()
        yloc = XYgrid[0, :, :, 1].reshape(-1).cpu()
...
def get_segLoss(op, target, spatWts, distMap, cond, alpha):
    ...
    for i in range(0, B):
        if cond[i] == 1:
            ...
            l_gD = GDiceLoss(
                op[i, ...].unsqueeze(0), target[i, ...].unsqueeze(0), F.softmax
            )
...
def GDiceLoss(ip, target, norm=F.softmax):
    ...
    target = torch.from_numpy(Label).cpu().to(dtype=ip.dtype)
...
def wCE(ip, target, spatWts):
    ...
    if rmIdx.size > 0:
        loss = spatWts.view(1, -1) * F.cross_entropy(
            ip.view(1, mxLabel, -1), target.view(1, -1), ignore_index=rmIdx.item()
        )
    else:
        loss = spatWts.view(1, -1) * F.cross_entropy(
            ip.view(1, mxLabel, -1), target.view(1, -1)
        )
...
def conf_Loss(x, gt, flag):
    ...
    if flag:
        ...
        loss = F.l1_loss(F.softmax(x, dim=1), torch.ones(B, C).cpu() / C)
    else:
...
def get_seg2elLoss(opSeg, opEl, loc_seg_ok):
    ...
    for i in range(0, B):
        if loc_seg_ok[i]:
            ...
            negmask = -posmask
            posmask = soft_heaviside(
                posmask, sc=64, mode=3
            )  # Positive outside the ellipse
            negmask = soft_heaviside(
                negmask, sc=64, mode=3
...
class WeightedHausdorffDistance(nn.Module):
    def __init__(self, resized_height, resized_width, p=-9, return_2_terms=False):
        ...
        self.resized_size = torch.tensor(
            [resized_height, resized_width], dtype=torch.float32
        )
    ...
    def forward(self, prob_map, gt, orig_sizes):
        ...
        for b in range(batch_size):
            ...
            if gt_b.ndimension() == 1 and (gt_b < 0).all().item() == 0:
                ...
                terms_2.append(
                    torch.tensor([self.max_dist], dtype=torch.get_default_dtype())
                )
                continue
