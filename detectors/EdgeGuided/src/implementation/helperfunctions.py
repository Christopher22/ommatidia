...
class my_ellipse:
    ...
    def phi2param(self, xm, ym):
        ...
        """
        try:
            x0 = (self.Phi[2] - self.Phi[3] * self.Phi[1]) / (
                (self.Phi[0]) - (self.Phi[1]) ** 2
            )
            y0 = (self.Phi[0] * self.Phi[3] - self.Phi[2] * self.Phi[1]) / (
                (self.Phi[0]) - (self.Phi[1]) ** 2
            )
        ...
        if (
            np.all(np.isreal(model))
            and np.all(~np.isnan(model))
            and np.all(~np.isinf(model))
        ):
            model = model
        else:
            model = [-1, -1, -1, -1, -1]
    ...
    def verify(self, pts):
        ...
        for i in range(0, N):
            err += (
                pts[i, :] @ self.mat @ pts[i, :].T
            )  # Note that the transpose here is irrelevant
    ...
    def generatePoints(self, N, mode):
        ...
        if mode == "equiSlope":
            ...
            x_2 = [
                ((a * b) ** 2) / (alpha + beta * K_fun(m) ** 2 - gamma * K_fun(m))
                for m in slope_list
            ]
...
class ElliFit:
    ...
    def fit(self):
        ...
        y = self.data[:, 1] - ym
        X = np.stack(
            [x**2, 2 * x * y, -2 * x, -2 * y, -np.ones((np.size(x),))], axis=1
        )
        Y = -(y**2)
        if self.weighted:
            self.Phi = np.linalg.inv(X.T.dot(np.diag(self.W)).dot(X)).dot(
    ...
    def fit_error(self, data):
        ...
        res = (
            (1 / self.model[2] ** 2) * (term1 - term2) ** 2
            + (1 / self.model[3] ** 2) * (term3 + term4) ** 2
            - 1
        )
        return np.abs(res)
...
def simple_string(ele):
    ...
    if type(ele) is list:
        ele = [
            "".join(e.lower() for e in str(string_ele) if e.isalnum())
...
def pad2Size(img, mask, elParam, pupil_c, size):
    ...
    elParam[0][:2] = (
        elParam[0][:2] + np.array([up_c, up_r]) if not badPup else elParam[0][:2]
    )
    elParam[1][:2] = (
        elParam[1][:2] + np.array([up_c, up_r]) if not badIri else elParam[1][:2]
    )
...
def getValidPoints(LabelMat, isPartSeg=True):
    ...
    for loc in zip(c, r):
        ...
        condPupil = (
            np.any(temp == 0) or np.any(temp == 1) or temp.size == 0
        )  # Not a valid pupil point
        if isPartSeg:
...
def plot_segmap_ellpreds(image, seg_map, pupil_ellipse, iris_ellipse):
    ...
    # f = open('ourdata/test1.log', 'w')
    # H, W = seg_map.shape
    # for i in range(H):
    #     for j in range(W):
    #         f.write('{} '.format(seg_map[i][j]))
    #     f.write('\n')
    # f.close()
    out_image = np.stack([image] * 3, axis=2)
    rr, cc = np.where(seg_map == 1)
    out_image[rr, cc, ...] = np.array([120, 183, 53])  # iris
    rr, cc = np.where(seg_map == 2)
    out_image[rr, cc, ...] = np.array([36, 231, 253])  # pupil
    # f = open('ourdata/test3.log', 'w')
    # # for k in range(3):
    # #     f.write('\n\n\n\n\n\n')
    # #     for i in range(H):
    # #         for j in range(W):
    # #             f.write('%03d ' % out_image[i][j][k])
    # #         f.write('\n')
    # for i in range(H):
    #     for j in range(W):
    #         if (seg_map[i][j] == 1):
    #             for k in range(3):
    #                 f.write('{} '.format(out_image[i][j][k]))
    #             f.write('\n')
    # f.close()

    loc_image_non_sat = image < (255 - 100)

    # print('pupil_ell ', pupil_ellipse)
    # print('iris_ell', iris_ellipse)
    # Add green to iris
    # print('tst')
    # out_image[..., 1] = out_image[..., 1] + 1000*loc_pupil#*loc_image_non_sat

    # Add yellow to pupilloc_iris
    # out_image[loc_iris][2] = 255
    # img_delta = 210 - out_image[..., 2]
    # out_image[..., 2] = out_image[..., 2] + img_delta*loc_iris#*loc_image_non_sat
    # img_delta = 255 - out_image[..., 0]
    # out_image[..., 0] = out_image[..., 0] + img_delta * loc_pupil  # *loc_image_non_sat
    # #out_image[..., 1] = out_image[..., 1] + 100*loc_pupil#*loc_image_non_sat
    # out_image = np.minimum(out_image, 255)
    # out_image = np.maximum(out_image, 0)
    # print(out_image.max(), out_image.min())
    # f = open('ourdata/test2.log', 'w')
    # # for k in range(3):
    # #     f.write('\n\n\n\n\n\n')
    # #     for i in range(H):
    # #         for j in range(W):
    ...
    # cv2.ellipse(out_image, (int(iris_ellipse[1]), int(iris_ellipse[0])), (int(iris_ellipse[3]), int(iris_ellipse[2])), iris_ellipse[4], 0, 360, (255, 255, 255), 3)  # 画椭圆
    ...
    #     out_image[rr_i, cc_i, ...] = np.array([0, 0, 255]) # BGR
    ...
    cv2.ellipse(
        out_image,
        (int(iris_ellipse[0]), int(iris_ellipse[1])),
        (int(iris_ellipse[2]), int(iris_ellipse[3])),
        iris_ellipse[4] / 3.14159 * 180,
        0,
        360,
        (255, 0, 0),
        1,
    ...
    cv2.ellipse(
        out_image,
        (int(pupil_ellipse[0]), int(pupil_ellipse[1])),
        (int(pupil_ellipse[2]), int(pupil_ellipse[3])),
        pupil_ellipse[4] / 3.14159 * 180,
        0,
        360,
        (0, 0, 255),
        1,
