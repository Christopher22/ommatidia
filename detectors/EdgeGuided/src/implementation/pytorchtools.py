...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model_dict):
        ...
        if self.verbose and self.mode == "min":
            print(
                "Validation metric decreased ({:.6f} --> {:.6f}).  Saving model ...".format(
                    self.val_loss_min, val_loss
                )
            )
        elif self.verbose and self.mode == "max":
            print(
                "Validation metric increased ({:.6f} --> {:.6f}).  Saving model ...".format(
                    self.val_loss_min, val_loss
                )
            )
...
def load_from_file(paths_file):
    ...
    for path in paths_file:
        try:
            netDict = torch.load(path, map_location=torch.device("cpu"))
