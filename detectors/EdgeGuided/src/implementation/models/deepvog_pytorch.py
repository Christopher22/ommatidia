    ...
    def forward(self, prev_feature_map, x):
        ...
        if self.up_sampling:
            ...
            x = nn.functional.interpolate(
                x, scale_factor=self.up_stride, mode="nearest"
            )
            # print (x.shape)
            x = self.conv2(x)
            x = self.bn2(x)
...
class DeepVOG_pytorch(nn.Module):
    def __init__(self, in_channels=3, out_channels=2, filter_size=(3, 3)):
        ...
        self.down_block1 = encoding_block(
            input_channels=in_channels,
            filter_size=filter_size,
            filters_num=self.output_channels,
            layer_num=1,
            block_type="down",
            stage=1,
            s=1,
        )
        self.down_block2 = encoding_block(
            input_channels=self.output_channels * 2,
            filter_size=filter_size,
            filters_num=self.output_channels * 2,
            layer_num=1,
            block_type="down",
            stage=1,
            s=1,
        )
        self.down_block3 = encoding_block(
            input_channels=self.output_channels * 4,
            filter_size=filter_size,
            filters_num=self.output_channels * 4,
            layer_num=1,
            block_type="down",
            stage=1,
            s=1,
        )
        self.down_block4 = encoding_block(
            input_channels=self.output_channels * 8,
            filter_size=filter_size,
            filters_num=self.output_channels * 8,
            layer_num=1,
            block_type="down",
            stage=1,
            s=1,
        ...
        self.up_block1 = decoding_block(
            skip_channels=0,
            input_channels=self.output_channels * 16,
            filter_size=filter_size,
            filters_num=self.output_channels * 16,
            layer_num=1,
            block_type="up",
            stage=1,
            s=1,
        )
        self.up_block2 = decoding_block(
            skip_channels=self.output_channels * 8,
            input_channels=self.output_channels * 16,
            filter_size=filter_size,
            filters_num=self.output_channels * 16,
            layer_num=1,
        ...
        self.up_block3 = decoding_block(
            skip_channels=self.output_channels * 4,
            input_channels=self.output_channels * 16,
            filter_size=filter_size,
            filters_num=self.output_channels * 8,
            layer_num=1,
        ...
        self.up_block4 = decoding_block(
            skip_channels=self.output_channels * 2,
            input_channels=self.output_channels * 8,
            filter_size=filter_size,
            filters_num=self.output_channels * 4,
            layer_num=1,
        ...
        self.up_block5 = decoding_block(
            skip_channels=self.output_channels,
            input_channels=self.output_channels * 4,
            filter_size=filter_size,
            filters_num=self.output_channels * 2,
            layer_num=1,
    ...
    def forward(
        ...
        elPred = torch.cat(
            [
                pred_c_seg_pup,
                torch.rand(B, 3).to(pred_c_seg_pup.device),
                pred_c_seg_pup,
                torch.rand(B, 3).to(pred_c_seg_pup.device),
            ],
            dim=1,
        )  # Bx5
        embedding = torch.ones(B, 5).cpu()  # Garbage
...
def get_allLoss(
    ...
    l_seg2pt_pup, pred_c_seg_pup = get_seg2ptLoss(
        op[:, 1, ...], normPts(pupil_center, target.shape[1:]), temperature=4
    )
    l_seg = 10 * F.cross_entropy(torch.softmax(op, dim=1), target, reduction="none")
    if torch.sum(log_seg_ok):
        l_seg = torch.sum(l_seg.reshape(B, -1).mean(dim=1) * log_seg_ok) / torch.sum(
            log_seg_ok
        )
    else:
        l_seg = 0.0
