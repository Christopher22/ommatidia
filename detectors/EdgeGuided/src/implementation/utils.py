...
def pri(ellipse):
    ...
    return "{:.2f} {:.2f} {:.2f} {:.2f} {:.2f}".format(
        ellipse[0], ellipse[1], ellipse[2], ellipse[3], angle
    )
...
def calc_ell_iou(seg, el_iris, mesh, nor=True, angle_nor=False):
    ...
    if angle_nor:
        el_iris[4] = el_iris[4] / 180.0 * 3.14159
    if not nor:
    ...
    # now2 = time.time()
    ell_map = torch.Tensor(ell_map).cpu()
    c = seg.mul(ell_map)
...
def generateImageGrid(
    ...
    for i in range(0, min(4, cond.shape[0])):
        ...
        if (not cond[i, 1]) or override:
            ...
            [rr_i_gt, cc_i_gt] = draw.ellipse_perimeter(
                int(el_iris_gt[1]),
                int(el_iris_gt[0]),
                int(el_iris_gt[3]),
                int(el_iris_gt[2]),
            ...
            [rr_p_gt, cc_p_gt] = draw.ellipse_perimeter(
                int(el_pupil_gt[1]),
                int(el_pupil_gt[0]),
                int(el_pupil_gt[3]),
                int(el_pupil_gt[2]),
...
def search_proper_parameter_iou(seg, elNorm, elNorm_gt, ell_para, ell_para_gt):
    ...
    ans = [ell_para[2], ell_para[3], ell_para[4] * 180.0 / 3.14159]
...
def generateImageGrid2(
    ...
    for i in range(0, min(4, cond.shape[0])):
        ...
        if (not cond[i, 1]) or override:
            ...
            # !!!!!!!! NEW
            [rr_new, cc_new] = draw.ellipse_perimeter(
                int(el_pupil_new[1]),
                int(el_pupil_new[0]),
                int(el_pupil_new[3]),
                int(el_pupil_new[2]),
        ...
        if (not cond[i, 0]) or override:
            ...
            rr, cc = draw.disk(
                (
                    pupil_center[i, 1].clip(6, im.shape[0] - 6),
                    pupil_center[i, 0].clip(6, im.shape[1] - 6),
                ),
                5,
            )
...
def calc_edge(args, img, edge_model, device):
    with torch.no_grad():
        ...
        img_edge = edge_model(
            torch.cat((img, img, img), dim=1).to(device).to(args.prec)
        )[-1]
