...
class RansacEllipse {
 public:
  ...
  int starburst_pupil_contour_detection(UINT8 *pupil_image,
                                        const cv::Point2d &startPoint,
                                        int width, int height, int edge_thresh,
                                        int N, int minimum_cadidate_features);
  int *pupil_fitting_inliers(UINT8 *pupil_image, int, int,
                             int &return_max_inliers);
 ...
 private:
  ...
  void locate_edge_points(const UINT8 *image, int width, int height, double cx,
                          double cy, int dis, double angle_step,
                          double angle_normal, double angle_spread,
                          int edge_thresh);
  cv::Point2d get_edge_mean();
  cv::Point2d *normalize_edge_point(double &dis_scale, cv::Point2d &nor_center,
                                    int ep_num);
  bool solve_ellipse(double *conic_param, double *ellipse_param);
  void denormalize_ellipse_param(double *par, double *normailized_par,
                                 double dis_scale, cv::Point2d nor_center);
...
class Starburst : public PupilDetectionMethod {
 public:
  ...
  int min_feature_candidates = 10;  // minimum number of pupil feature
                                    // candidates
  int corneal_reflection_ratio_to_image_size =
      2;  // approx max size of the reflection relative to image height ->
          // height/this
  ...
  Starburst()
      : ransacEllipse(),
        avgIntensityHori(nullptr),
        intensityFactorHori(nullptr),
        curH(0),
        startPoint(0, 0),
        lostFrameNum(0),
        imageSize(0, 0) {
 ...
 private:
  ...
  double
      *intensityFactorHori;  // horizontal intensity factor for noise reduction
  int curH;
  int lostFrameNum;
