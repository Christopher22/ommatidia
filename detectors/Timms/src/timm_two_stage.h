...
// #include "helpers.h"
// Skip the unecessary visualization
...
class Timm_two_stage {
 ...
 public:
  ...
  Timm_two_stage()
#ifdef __TIMM_OPENCL__
      : stage1(gradient_kernel),
        stage2(gradient_kernel)
#endif
  {
  ...
  void set_options(options o) {
    ...
    // if the window width is smaller than the down scaling width, make the
    // down_scaling_width equal to the window width to save processing time
    stage2.opt.down_scaling_width =
        std::min(stage2.opt.down_scaling_width, opt.window_width);
  }

  // std::array<float, 4> get_timings() { return std::array<float,
  // 4>{stage1.measure_timings[0], stage1.measure_timings[1],
 ...
 private:
  ...
  /*void draw_cross(cv::Mat& img, cv::Point p, int w,
  ...
  cv::Rect fit_rectangle(cv::Mat frame, cv::Point2f c, int w) {
    ...
    int x = c.x;
    x = clip<int>(x, w2, frame.cols - w2);
    int y = c.y;
    y = clip<int>(y, w2, frame.rows - w2);
    return cv::Rect(x - w2, y - w2, 2 * w2, 2 * w2);
  }

 public:
  void visualize_frame(cv::Mat& frame, cv::Point2f pupil_pos,
                       cv::Point2f pupil_pos_coarse,
                       const cv::Point2f* ground_truth_pos = nullptr) {
    ...
    if (ground_truth_pos) {
      draw_cross(frame, cv::Point(*ground_truth_pos), 7,
                 cv::Scalar(255, 0, 255));
    }*/
  }
};
