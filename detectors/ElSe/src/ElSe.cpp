/*
  ...
  Environments", W. Fuhl, T. C. Santini, T. C. Kübler, E. Kasneci ETRA 2016 :
  Eye Tracking Research and Application 2016
...
static int calc_inner_gray(Mat *pic, std::vector<Point> curve,
                           RotatedRect ellipse) {
  int gray_val = 0;
  int gray_cnt = 0;
...
static std::vector<std::vector<Point>> get_curves(Mat *pic, Mat *edge,
                                                  Mat *magni, int start_x,
                                                  int end_x, int start_y,
                                                  int end_y, double mean_dist,
  ...
  for (unsigned int iii = 0; iii < all_lines.size(); iii++) {
    ...
    if (add_curve) {
      ...
      if (add_curve) {
        if (!is_good_ellipse_eval(&ellipse, pic, &results)) add_curve = false;
      }
    }
    ...
    if (add_curve) {
      if (inner_color_range >= 0) {
        ...
        mean_inner_gray =
            (int)(mean_inner_gray *
                  (1 + abs(ellipse.size.height - ellipse.size.width)));
...
static RotatedRect find_best_edge(Mat *pic, Mat *edge, Mat *magni, int start_x,
  ...
  std::vector<std::vector<Point>> all_curves =
      get_curves(pic, edge, magni, start_x, end_x, start_y, end_y, mean_dist,
                 inner_color_range);
...
static void filter_edges(Mat *edge, int start_xx, int end_xx, int start_yy,
                         int end_yy) {
  ...
  for (int j = start_y; j < end_y; j++)
    for (int i = start_x; i < end_x; i++) {
      ...
      for (int k1 = -1; k1 < 2; k1++)
        for (int k2 = -1; k2 < 2; k2++) {
          if (edge->data[(edge->cols * (j + k1)) + (i + k2)] > 0) neig++;
        }

      if (neig > 3) edge->data[(edge->cols * (j)) + (i)] = 0;
    }
    ...
    for (int j = 0; j < sz_x; j++) {
      ...
      for (int ii = -fak; ii <= fak; ii++)
        for (int jj = -fak; jj <= fak; jj++) {
          if (idy + ii > 0 && idy + ii < pic->rows && idx + jj > 0 &&
              idx + jj < pic->cols) {
            if ((unsigned int)pic->data[(pic->cols * (idy + ii)) + (idx + jj)] >
                255)
...
static RotatedRect blob_finder(Mat *pic) {
  ...
  int fak_mum = 5;
  int fakk =
      pic->cols > pic->rows ? (pic->cols / 100) + 1 : (pic->rows / 100) + 1;
  ...
  for (int i = 0; i < result.rows; i++) {
    ...
    for (int j = 0; j < result.cols; j++) {
      if (p_res[j] < 0) p_res[j] = 0;
    }
  }

  filter2D(img, result_neg, -1, blob_mat_neg, Point(-1, -1), 0,
           BORDER_REPLICATE);
  ...
  if (pos.y > 0 && pos.y < pic->rows && pos.x > 0 && pos.x < pic->cols) {
    ...
    for (int i = -(2); i < (2); i++) {
      for (int j = -(2); j < (2); j++) {
        if (pos.y + i > 0 && pos.y + i < pic->rows && pos.x + j > 0 &&
            pos.x + j < pic->cols) {
...
Pupil ElSe::run(const Mat &frame) {
  ...
  ellipse = find_best_edge(&pic, &detected_edges, &magni, start_x, end_x,
                           start_y, end_y, mean_dist, inner_color_range);
...
void ElSe::run(const cv::Mat &frame, const cv::Rect &roi, Pupil &pupil,
               const float &minPupilDiameterPx = -1,
               const float &maxPupilDiameterPx = -1) {
  if (roi.area() < 10) {
