/*
  ...
  "ExCuSe: Robust Pupil Detection in Real-World Scenarios", W. Fuhl, T. C.
  Kübler, K. Sippel, W. Rosenstiel, E. Kasneci CAIP 2015 : Computer Analysis of
  Images and Patterns
          ...
          if (akt_pos - pic_x - 1 >= 0 && akt_pos + pic_x + 1 < pic_x * pic_y) {
            for (int k1 = -1; k1 < 2; k1++)
              for (int k2 = -1; k2 < 2; k2++) {
                if (check->data[(akt_pos + (k1 * pic_x)) + k2] == 0 &&
                    weak->data[(akt_pos + (k1 * pic_x)) + k2] != 0) {
...
static cv::Mat canny_impl(cv::Mat *pic) {
  ...
  float gau[16] = {0.000000220358050f, 0.000007297256405f, 0.000146569312970f,
                   0.001785579770079f, 0.013193749090229f, 0.059130281094460f,
                   0.160732768610747f, 0.265003534507060f, 0.265003534507060f,
                   0.160732768610747f, 0.059130281094460f, 0.013193749090229f,
                   0.001785579770079f, 0.000146569312970f, 0.000007297256405f,
                   0.000000220358050f};
  float deriv_gau[16] = {
      -0.000026704586264f, -0.000276122963398f, -0.003355163265098f,
      -0.024616683775044f, -0.108194751875585f, -0.278368310241814f,
      -0.388430056419619f, -0.196732206873178f, 0.196732206873178f,
      0.388430056419619f,  0.278368310241814f,  0.108194751875585f,
      0.024616683775044f,  0.003355163265098f,  0.000276122963398f,
      0.000026704586264f};
  ...
  filter2D(res_x, res_x, ddepth, deriv_gau_x, anchor, delta,
           cv::BORDER_REPLICATE);

  filter2D(*pic, res_y, ddepth, gau_x, anchor, delta, cv::BORDER_REPLICATE);

  cv::transpose(res_y, res_y);
  filter2D(res_y, res_y, ddepth, deriv_gau_x, anchor, delta,
           cv::BORDER_REPLICATE);
  cv::transpose(res_y, res_y);
  ...
  for (int i = 0; i < res.rows; i++) {
    ...
    for (int j = 0; j < res.cols; j++) {
      // res.at<float>(j, i)= sqrt( (res_x.at<float>(j, i)*res_x.at<float>(j,
      // i)) + (res_y.at<float>(j, i)*res_y.at<float>(j, i)) ); res.at<float>(j,
      // i)=robust_pytagoras_after_MOLAR_MORRIS(res_x.at<float>(j, i),
      // res_y.at<float>(j, i)); res.at<float>(j, i)=hypot(res_x.at<float>(j,
      // i), res_y.at<float>(j, i));
    ...
    for (int j = 1; j < res.cols - 1; j++) {
      ...
      if ((iy <= 0 && ix > -iy) || (iy >= 0 && ix < -iy)) {
        ...
        if (p_res[j] >= grad1 && p_res[j] >= grad2) {
          ...
          if (p_res[j] > high_th) p_non_ms_hth[j] = (char)255;
        }
      }
...
static bool peek(cv::Mat *pic, double *stddev, int start_x, int end_x,
                 int start_y, int end_y, int peek_detector_factor,
  ...
  *stddev = *stddev / ((end_x - start_x));
  ...
  if (max_gray > (mean_gray * peek_detector_factor) &&
      max_gray_pos > bright_region_th)
    return true;
  else
    return false;
}

static void remove_points_with_low_angle(cv::Mat *edge, int start_xx,
                                         int end_xx, int start_yy, int end_yy) {
    ...
    for (int i = start_x; i < end_x; i++) {
      if ((int)edge->data[(edge->cols * (j)) + (i)]) {
        ...
        for (int k = 0; k < 8 && !valid; k++)
          // if( box[k] && (box[(k+3)%8] || box[(k+4)%8] || box[(k+5)%8]) )
          // valid=true;
          if (box[k] &&
              (box[(k + 2) % 8] || box[(k + 3) % 8] || box[(k + 4) % 8] ||
               box[(k + 5) % 8] || box[(k + 6) % 8]))
            valid = true;
  ...
  for (int j = start_y; j < end_y; j++)
    for (int i = start_x; i < end_x; i++) {
      ...
      if (box[4]) {
        ...
        // if( (box[1] && box[5]) || (box[1] && box[3]) || (box[3] && box[7]) ||
        // (box[5] && box[7]) ) 		edge->data[(edge->cols*(j))+(i)]=0;
      }
    }
  ...
  for (int j = start_y; j < end_y; j++)
    for (int i = start_x; i < end_x; i++) {
      ...
      if (box[4]) {
        ...
        if ((box[14] && !box[7] && !box[10]) &&
            ((box[8] || box[6]) && (box[16] || box[15]))) {
          edge->data[(edge->cols * (j + 1)) + (i + 1)] = 0;
          edge->data[(edge->cols * (j + 1)) + (i - 1)] = 0;
        ...
        if ((box[11] && !box[5] && !box[9]) &&
            ((box[8] || box[2]) && (box[13] || box[12]))) {
          edge->data[(edge->cols * (j + 1)) + (i + 1)] = 0;
          edge->data[(edge->cols * (j - 1)) + (i + 1)] = 0;
...
static std::vector<std::vector<cv::Point>> get_curves(
    cv::Mat *pic, cv::Mat *edge, int start_x, int end_x, int start_y, int end_y,
  ...
  for (int i = 0; i < IMG_SIZE; i++)
    for (int j = 0; j < IMG_SIZE; j++) check[i][j] = 0;
    ...
    for (int j = start_y; j < end_y; j++) {
      if (edge->data[(edge->cols * (j)) + (i)] == 255 && !check[i][j]) {
        ...
        while (akt_idx < curve_idx) {
          ...
          for (int k1 = -1; k1 < 2; k1++)
            for (int k2 = -1; k2 < 2; k2++) {
              if (akt_pos.x + k1 >= start_x && akt_pos.x + k1 < end_x &&
                  akt_pos.y + k2 >= start_y && akt_pos.y + k2 < end_y)
                if (!check[akt_pos.x + k1][akt_pos.y + k2])
                  if (edge->data[(edge->cols * (akt_pos.y + k2)) +
                                 (akt_pos.x + k1)] == 255) {
        ...
        if (curve_idx > 0 && curve.size() > 0) {
          ...
          for (int i = 0; i < curve.size(); i++)
            if (abs(mean_p.x - curve[i].x) <= mean_dist &&
                abs(mean_p.y - curve[i].y) <= mean_dist)
              add_curve = false;
          ...
          if (add_curve) {
            if (inner_color_range > 0) {
              ...
              // calc inner mean
              for (int i = 0; i < curve.size(); i++) {
                if (pic->data[(pic->cols * (curve[i].y + 1)) + (curve[i].x)] !=
                        0 ||
                    pic->data[(pic->cols * (curve[i].y - 1)) + (curve[i].x)] !=
                        0)
                  if (sqrt(pow(double(curve[i].y - mean_p.y), 2) +
                    ...
                    mean_inner_gray +=
                        (unsigned char)pic->data[(pic->cols * (curve[i].y)) +
                                                 (curve[i].x + 2)];
                  else
                    mean_inner_gray +=
                        (unsigned char)pic->data[(pic->cols * (curve[i].y)) +
                ...
                else if (pic->data[(pic->cols * (curve[i].y)) +
                                   (curve[i].x + 1)] != 0 ||
                         pic->data[(pic->cols * (curve[i].y)) +
                                   (curve[i].x - 1)] != 0)
                ...
                else if (pic->data[(pic->cols * (curve[i].y + 1)) +
                                   (curve[i].x + 1)] != 0 ||
                         pic->data[(pic->cols * (curve[i].y - 1)) +
                                   (curve[i].x - 1)] != 0)
                  if (sqrt(pow(double(curve[i].y - mean_p.y - 2), 2) +
                           pow(double(curve[i].x - mean_p.x + 2), 2)) <
                      sqrt(pow(double(curve[i].y - mean_p.y + 2), 2) +
                ...
                // mean_inner_gray+=pic->data[( pic->cols*(
                // curve[i].y+((mean_p.y-curve[i].y)/2) ) ) + (
                // curve[i].x+((mean_p.x-curve[i].x)/2) )];
              }

              mean_inner_gray =
                  floor((double(mean_inner_gray) / double(curve.size())) + 0.5);

              if (mean_inner_gray_last >
                  (mean_inner_gray + inner_color_range)) {
                mean_inner_gray_last = mean_inner_gray;
                all_curves.clear();
                all_curves.push_back(curve);
              } else if (mean_inner_gray_last <=
                             (mean_inner_gray + inner_color_range) &&
                         mean_inner_gray_last >=
                             (mean_inner_gray - inner_color_range)) {
...
static cv::RotatedRect find_best_edge(cv::Mat *pic, cv::Mat *edge, int start_x,
  ...
  std::vector<std::vector<cv::Point>> all_curves = get_curves(
      pic, edge, start_x, end_x, start_y, end_y, mean_dist, inner_color_range);
...
static int calc_pos(int *hist, int mini, int max_region_hole,
                    int min_region_size, int real_hist_sz) {
  int pos = 0;
  ...
  for (int i = 0; i < DEF_SIZE; i++) {
    if (hist[i] > mini && !region_start) {
      ...
      pos_hole += i;
               ...
               hole_size >= max_region_hole && count >= min_region_size) {
      if (count < 1) count = 1;
      mean_pos = mean_pos / count;
      if (pow(double((real_hist_sz / 2) - mean_pos), 2) <
          pow(double((real_hist_sz / 2) - pos), 2))
        pos = mean_pos;
...
static cv::Point th_angular_histo(cv::Mat *pic, cv::Mat *pic_th, int start_x,
                                  int end_x, int start_y, int end_y, int th,
                                  double th_histo, int max_region_hole,
                                  int min_region_size) {
  cv::Point pos(0, 0);
  ...
  for (int i = start_x; i < end_x; i++) {
    for (int j = start_y; j < end_y; j++) {
      if (pic->data[(pic->cols * j) + i] < th) {
        ...
        if (j >= 0 && j < DEF_SIZE && i >= 0 && i < DEF_SIZE && idx_lb >= 0 &&
            idx_lb < DEF_SIZE && idx_br >= 0 && idx_br < DEF_SIZE) {
  ...
  pos_lb = calc_pos(hist_lb, min_lb, max_region_hole, min_region_size,
                    pic->cols + pic->rows);
  pos_b = calc_pos(hist_b, min_b, max_region_hole, min_region_size, pic->cols);
  pos_br = calc_pos(hist_br, min_br, max_region_hole, min_region_size,
                    pic->cols + pic->rows);
  ...
  */
  ...
  if (pos_l > 0 && pos_lb > 0 && pos_b > 0 && pos_br > 0) {
    pos.x =
        floor(((pos_b + (floor((((pos_br + pic->rows) - pos_lb) / 2) + 0.5) +
                         pos_lb - pic->rows)) /
               2) +
              0.5);
    pos.y =
        floor(((pos_l + (pic->rows -
                         floor((((pos_br + pic->rows) - pos_lb) / 2) + 0.5))) /
               2) +
              0.5);
    ...
    pos.x =
        floor((((pos_br + pic->rows) - pos_lb) / 2) + 0.5) + pos_lb - pic->rows;
  ...
  imshow("th",*pic_th);
  cv::ellipse(*pic, cv::RotatedRect(pos, cv::Size2f(5,5),0),
  CV_RGB(255,255,255)); imshow("angular",*pic);
  */

  return pos;
}

static void grow_region(cv::RotatedRect *ellipse, cv::Mat *pic,
  ...
  for (int i = -2; i < 3; i++)
    for (int j = -2; j < 3; j++) {
      if (y0 + j > 0 && y0 + j < pic->rows && x0 + i > 0 &&
          x0 + i < pic->cols) {
  ...
  for (int i = 1; i < max_ellipse_radi; i++) {
    ...
    if (p_top < 0.2 && p_bottom < 0.2) break;

    if (p_left < 0.2 && p_right < 0.2) break;
  }
  ...
  while(all_p_idx<all_points.size()){
      ...
      for(int i=-1;i<2;i++)
          for(int j=-1;j<2;j++){
              ...
              if(ak_p.y+j>0 && ak_p.y+j<pic->rows && ak_p.x+i>0 &&
  ak_p.x+i<pic->cols)
...
static bool is_good_ellipse(cv::RotatedRect *ellipse, cv::Mat *pic,
                            int good_ellipse_threshold, int max_ellipse_radi) {
  if (ellipse->center.x == 0 && ellipse->center.y == 0) return false;
...
static void rays(cv::Mat *th_edges, int end_x, int end_y, cv::Point *pos,
                 int *ret) {
  for (int i = 0; i < 8; i++) ret[i] = -1;
  for (int i = 0; i < end_x; i++)
    for (int j = 0; j < end_y; j++) {
      if (pos->x - i > 0 && pos->x + i < th_edges->cols && pos->y - j > 0 &&
          pos->y + j < th_edges->rows) {
        if ((int)th_edges->data[(th_edges->cols * (pos->y)) + (pos->x + i)] !=
                0 &&
            ret[0] == -1) {
          ret[0] =
              th_edges->data[(th_edges->cols * (pos->y)) + (pos->x + i)] - 1;
          // std::cout<<"val:"<<ret[0]<<" x:"<<pos->x+i<<"
          // y:"<<pos->y<<std::endl;
        }
        if ((int)th_edges->data[(th_edges->cols * (pos->y)) + (pos->x - i)] !=
                0 &&
        ...
        if ((int)th_edges->data[(th_edges->cols * (pos->y + j)) + (pos->x)] !=
                0 &&
            ret[2] == -1) {
          ret[2] =
              th_edges->data[(th_edges->cols * (pos->y + j)) + (pos->x)] - 1;
          // std::cout<<"val:"<<ret[0]<<" x:"<<pos->x<<"
          // y:"<<pos->y+j<<std::endl;
        }
        if ((int)th_edges->data[(th_edges->cols * (pos->y - j)) + (pos->x)] !=
                0 &&
        ...
        if ((int)th_edges->data[(th_edges->cols * (pos->y + j)) +
                                (pos->x + i)] != 0 &&
            ret[4] == -1 && i == j) {
          ret[4] =
              th_edges->data[(th_edges->cols * (pos->y + j)) + (pos->x + i)] -
              1;
          // std::cout<<"val:"<<ret[0]<<" x:"<<pos->x+i<<"
          // y:"<<pos->y+j<<std::endl;
        }
...
static void zero_around_region_th_border(cv::Mat *pic, cv::Mat *edges,
                                         cv::Mat *th_edges, int th,
                                         int edge_to_th, double mean_dist,
  ...
  // std::cout<<"sx:"<<start_x<<" sy:"<<start_y<<" ex:"<<end_x<<" ey:"<<end_y<<"
  // dist:"<<edge_to_th<<std::endl;
  for (int i = start_x; i < end_x; i++)
    for (int j = start_y; j < end_y; j++) {
      if (pic->data[(pic->cols * j) + (i)] < th) {
        for (int k1 = -edge_to_th; k1 < edge_to_th; k1++)
          for (int k2 = -edge_to_th; k2 < edge_to_th; k2++) {
            if (i + k1 >= 0 && i + k1 < pic->cols && j + k2 > 0 &&
                j + k2 < edges->rows)
...
for(int i=start_x-edge_to_th; i<end_x+edge_to_th; i++)
    ...
    */
    ...
    for (int i = 0; i < all_curves.size(); i++) {
      ...
      for (int j = 0; j < all_curves[i].size(); j++) {
        if (all_curves[i][j].x >= 0 && all_curves[i][j].x < th_edges->cols &&
            all_curves[i][j].y >= 0 && all_curves[i][j].y < th_edges->rows)
          th_edges->data[(th_edges->cols * (all_curves[i][j].y)) +
                         (all_curves[i][j].x)] =
    ...
    if (selected_points.size() > 5) {
      *pos = cv::fitEllipse(cv::Mat(selected_points));
      /*
      cv::ellipse(*pic, cv::RotatedRect(ellipse.operator
      CvBox2D()),CV_RGB(255,255,255)); cv::imshow("akt", *pic);
      */
    }
  }
  ...
  std::cout<<pos->x<<";"<<pos->y<<std::endl;
  cv::ellipse(*pic, cv::RotatedRect(*pos, cv::Size2f(5,5),0),
  CV_RGB(255,255,255)); imshow("opt",*pic);
  */
}
...
static void optimize_pos(cv::Mat *pic, double area, cv::Point *pos) {
  ...
  int reg_size = sqrt(sqrt(pow(double(area * pic->cols * 2), 2) +
                           pow(double(area * pic->rows * 2), 2)));
...
static cv::RotatedRect runexcuse(cv::Mat *pic, cv::Mat *pic_th,
                                 cv::Mat *th_edges, int good_ellipse_threshold,
                                 int max_ellipse_radi) {
  // mean under mean
  ...
  peek_found = peek(pic, &stddev, start_x, end_x, start_y, end_y,
                    peek_detector_factor, bright_region_th);
  ...
  for (int i = 0; i < picpic.cols; i++)
    for (int j = 0; j < picpic.rows; j++) {
      picpic.data[(picpic.cols * j) + i] =
          pic->data[(pic->cols * (start_y + j)) + (start_x + i)];
    }
  ...
  for (int i = 0; i < detected_edges2.cols; i++)
    for (int j = 0; j < detected_edges2.rows; j++) {
      detected_edges
          .data[(detected_edges.cols * (start_y + j)) + (start_x + i)] =
  ...
  if (peek_found) {
    ...
    ellipse = find_best_edge(pic, &detected_edges, start_x, end_x, start_y,
                             end_y, mean_dist, inner_color_range);

    if (ellipse.center.x <= 0 || ellipse.center.x >= pic->cols ||
        ellipse.center.y <= 0 || ellipse.center.y >= pic->rows) {
      ellipse.center.x = 0;
      ellipse.center.y = 0;
      ellipse.angle = 0.0;
      ellipse.size.height = 0.0;
  ...
  if (!peek_found) {
    pos = th_angular_histo(pic, pic_th, start_x, end_x, start_y, end_y,
                           threshold_up, th_histo, max_region_hole,
                           min_region_size);
  ...
  if (pos.x > 0 && pos.y > 0 && pos.x < pic->cols && pos.y < pic->rows &&
      !peek_found) {
    ...
    ellipse.size.height = 0.0;
    ellipse.size.width = 0.0;
    zero_around_region_th_border(pic, &detected_edges, th_edges, threshold_up,
                                 edge_to_th, mean_dist, area_edges, &ellipse);
  }
...
Pupil ExCuSe::run(const Mat &frame) {
  ...
  if (frame.rows > IMG_SIZE || frame.cols > IMG_SIZE) {
    ...
    cv::resize(frame, downscaled, Size(), scalingRatio, scalingRatio,
               INTER_LINEAR);
  }
  ...
  cv::RotatedRect ellipse = runexcuse(&target, &pic_th, &th_edges,
                                      good_ellipse_threshold, max_ellipse_radi);
  cv::RotatedRect scaledEllipse(cv::Point2f(ellipse.center.x / scalingRatio,
                                            ellipse.center.y / scalingRatio),
                                cv::Size2f(ellipse.size.width / scalingRatio,
                                           ellipse.size.height / scalingRatio),
                                ellipse.angle);

  return Pupil(scaledEllipse);
}

void ExCuSe::run(const cv::Mat &frame, const Rect &roi, Pupil &pupil,
  ...
  pupil = run(frame(roi));
  if (pupil.center.x > 0 && pupil.center.y > 0) pupil.shift(roi.tl());
}
