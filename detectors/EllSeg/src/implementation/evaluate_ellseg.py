            ...
            if op_shape[0] > img.shape[0]:
                ...
                if pad_width % 2 == 0:
                    ...
                    img = np.pad(
                        img,
                        (
                            (int(np.floor(pad_width / 2)), int(np.ceil(pad_width / 2))),
                            (0, 0),
                        ),
                    )
...
def evaluate_ellseg_on_image(frame, model, args):
    ...
    if ellipse_from_network:
        ...
        _, norm_pupil_center = get_seg2ptLoss(
            seg_out[:, 2, ...],
            torch.zeros(
                2,
            ),
        ...
        _, norm_iris_center = get_seg2ptLoss(
            -seg_out[:, 0, ...],
            torch.zeros(
                2,
            ),
...
def evaluate_ellseg_per_video(path_vid, args, model):
    ...
    while ret:
        ...
        seg_map, latent, pupil_ellipse, iris_ellipse = evaluate_ellseg_on_image(
            input_tensor, model
        )

        # Return ellipse predictions back to original dimensions
        seg_map, pupil_ellipse, iris_ellipse = rescale_to_original(
            seg_map, pupil_ellipse, iris_ellipse, scale_shift, frame.shape
        )
