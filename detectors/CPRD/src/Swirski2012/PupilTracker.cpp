...
struct section_guard {
  ...
  section_guard(const std::string& name, pupiltracker::tracker_log& log)
      : name(name), log(log), t() {}
  ~section_guard() { log.add(name, t); }
  operator bool() const { return false; }
};

inline section_guard make_section_guard(const std::string& name,
                                        pupiltracker::tracker_log& log) {
...
#define SECTION(A, B)                                                    \
  if (const section_guard& _section_guard_ = make_section_guard(A, B)) { \
  } else
//ֱ����code��ʵ�ַ����滻������SECTION{}�е�{}��else�Ĳ��֣�if()�ڲ���bool
...
class HaarSurroundFeature {
 public:
  HaarSurroundFeature(int r1, int r2) : r_inner(r1), r_outer(r2) {
    ...
    // val_inner = std::sqrt( (double)count_outer/(count_inner*count_outer +
    // sq(count_inner)) ); val_outer = -std::sqrt(
    // (double)count_inner/(count_inner*count_outer + sq(count_outer)) );

    // Square radius normalised values
    //
    // Want the response to be scale-invariant, so scale it by the number of
    // pixels inside it:
    ...
    val_outer = -val_inner * count_inner /
                count_outer;  // val_inner*count_inner = 1
                              //���������Ȩ�أ�ʵ���Ͼ��Ǽ���ƽ��ֵ�Ĳ�ֵ
  }
...
cv::RotatedRect fitEllipse(
    const std::vector<pupiltracker::EdgePoint>& edgePoints) {
...
bool pupiltracker::findPupilEllipse(const pupiltracker::TrackerParams& params,
                                    const cv::Mat& m,
  ...
  SECTION("Haar responses", log) {
    ...
    for (int r = params.Radius_Min; r < params.Radius_Max; r += rstep) {
      ...
      //�����ӵĴ��룬��Ҫ�ǿ��ǵ����õ�params.Radius_Max���󣬳�����Χ�ᱨ��
      if (((mEye.rows - r - r - 1) / ystep + 1) / 8 <= 0) continue;
          ...
          [&](tbb::blocked_range<int> range,
            ...
            for (int i = range.begin(), y = r + range.begin() * ystep;
              ...
              for (int x = r; x < mEye.cols - r; x += xstep) {
                int sumInner =
                    *p00_inner + *p11_inner - *p01_inner - *p10_inner;
                int sumOuter = *p00_outer + *p11_outer - *p01_outer -
                               *p10_outer - sumInner;

                double response =
                    f.val_inner * sumInner + f.val_outer * sumOuter;
  ...
  // ---------------------------
  // pHaarPupil��rect�����ģ��ú�������������Rect
  cv::Rect roiHaarPupil =
      cvx::roiAround(cv::Point(pHaarPupil.x, pHaarPupil.y), haarRadius);
  ...
  //����centers��һ��threshold��ʵ�ǵ�Ч�ģ�������������ͻ���ȫ����ֵ��������һ��
  float threshold;
  SECTION("KMeans", log) {
    ...
    for (int i = 0; i < 2; i++)  //����ѭ��������kmeans��ʼ��
    {
      cv::Mat_<uchar> labels;
      float centres[2] = {candidate0[i], candidate1[i]};
      float dist =
          cvx::histKmeans(hist, 0, 256, 2, centres, labels,
  ...
  SECTION("Find best region", log) {
    ...
    elPupilThresh = cvx::fitEllipse(momentsPupilThresh);  //�ò���������Ĺ���

    // Shift best region into eye coords (instead of pupil region coords), and
    // get ROI
  ...
  out.bbPupilThresh = bbPupilThresh;  //����bounding box
  out.elPupilThresh = elPupilThresh;  //����min area rect
  ...
  SECTION("Pupil preprocessing", log) {
    ...
    if (params.CannyBlur > 0) {
      cv::GaussianBlur(mPupilOpened, mPupilBlurred, cv::Size(),
                       params.CannyBlur);
    } else {
  ...
  if (params.StarburstPoints > 0) {
    SECTION("Starburst", log) {
      // Starburst from initial pupil approximation, stopping when an edge is
      // hit. Collect all edge points into a vector

      // The initial pupil approximations are:
      //    Centre of mass of thresholded region
      //    Halfway along the major axis (calculated form second moments) in
      //    each direction
      ...
      if (edgePoints.size() < params.StarburstPoints / 2) return false;
    }
  } else {
  ...
  SECTION("Ellipse fitting", log) {
    ...
    if (params.PercentageInliers == 0) return false;
    ...
    if (edgePoints.size() >= n)  // Minimum points for ellipse
    {
      ...
      struct EllipseRansac {
        ...
        void operator()(const tbb::blocked_range<size_t>& r) {
          ...
          // std::cout << "Ransac start (" << (r.end()-r.begin()) << "
          // elements)" << std::endl;
      ...
      EllipseRansac ransac(params, edgePoints, n, bbPupil, out.mPupilSobelX,
                           out.mPupilSobelY);
      try {
