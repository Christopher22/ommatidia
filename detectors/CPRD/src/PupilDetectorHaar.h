...
@param img_gray input image: UINT8 [0, 255]. If img_gray is float [0,1], we can
add a function to transfrom it to [0,255]. But it is not recommended, as [0,255]
has better significance than [0,1].
*/
class PupilDetectorHaar {
 public:
  PupilDetectorHaar()
      : pupil_rect_coarse_(Rect()),
        outer_rect_coarse_(Rect()),
  ...
  void detect(const Mat& img_gray) {
    ...
    Mat img_down;
    {
      ...
      resize(img_gray, img_down,
             Size(img_gray.cols / ratio_downsample_,
    ...
    drawCoarse(img_coarse_, pupil_rect_coarse_, outer_rect_coarse_,
               max_response_coarse_, RED);

    //-------------------------- 3 Fine Detection --------------------------
    /* No fine detection for poor image quality when the intensity in the pupil
    region is close to, or even greater than, the intensity in the surrounding
    region.
    */
    ...
    // upsample
    pupil_rect_coarse_ =
        rectScale(pupil_rect_coarse_, ratio_downsample_, false);
    outer_rect_coarse_ =
        rectScale(outer_rect_coarse_, ratio_downsample_, false);
    pupil_rect_fine_ = rectScale(pupil_rect_fine_, ratio_downsample_, false);

    center_coarse_ =
        Point2f(pupil_rect_coarse_.x + pupil_rect_coarse_.width * 1.0f / 2,
                pupil_rect_coarse_.y + pupil_rect_coarse_.height * 1.0f / 2);
    center_fine_ =
        Point2f(pupil_rect_fine_.x + pupil_rect_fine_.width * 1.0f / 2,
  ...
  void fineDetection(const Mat& img_down, const Rect& pupil_rect_coarse,
                     Rect& pupil_rect_fine);

  // plot on the original img_BGR.
  void drawCoarse(Mat& img_BGR, Rect pupil_rect, Rect outer_rect,
                  double max_response, Scalar color = RED) {
    int thickness = 1;
    rectangle(img_BGR, pupil_rect, color, thickness, 8);
    rectangle(img_BGR, outer_rect, color, thickness, 8);
    Point center = Point(pupil_rect.x + pupil_rect.width / 2,
                         pupil_rect.y + pupil_rect.height / 2);
  ...
  void drawCoarse(Mat& img_BGR) {
    drawCoarse(img_BGR, pupil_rect_coarse_, outer_rect_coarse_,
               max_response_coarse_, RED);
  }
  ...
  static void filterLight(const Mat& img_gray, Mat& img_blur, int tau) {
    ...
    ////measureTime([&]() {bilateralFilter(img_blur, temp, 5, 100, 1, 4); },
    ///"bilateral\t");
    // measureTime([&]() {pyrMeanShiftFiltering(temp, temp, 20, 20, 2); }, "mean
    // shift\t"); cvtColor(temp, img_blur, CV_BGR2GRAY);
  ...
  bool extractEllipse(const Mat& img_gray, const Rect& pupil_rect,
                      cv::RotatedRect& ellipse_rect, Point2f& center_fitting) {
    ...
    if (pupil_rect_fine_.contains(ellipse_rect.center) &&
        (ellipse_rect.size.width > 0)) {
 ...
 private:
  ...
  void areaSort(Mat& area, Mat& index) {
    ...
    for (int i = 0; i < area.rows - 1; i++) index.at<int>(i) = i;
  ...
  */
  double getResponseMap(const Mat& integral_img, int width, int height,
                        double ratio, bool useSquareHaar, double kf, Rect roi,
                        int xystep, Rect& pupil_rect, Rect& outer_rect,
                        double& mu_inner, double& mu_outer);
  ...
  */
  double getResponseValue(const Mat& integral_img, Rect& inner_rect,
                          Rect& outer_rect, double kf, double& mu_inner,
                          double& mu_outer) {
    iterate_count_ += 1;
    ...
    mu_outer =
        1.0 * (outer_bII - inner_bII) / (outer_rect.area() - inner_rect.area());
  ...
  */
  ...
  void rectSuppression2(vector<Rect>& rectlist, vector<double>& response,
                        vector<Rect>& rectlist_out,
                        vector<double>& response_out) {
 ...
 public:
  ...
  Rect init_rect_ =
      Rect(0, 0, 0, 0);  // based on original image resolution, not downsample
