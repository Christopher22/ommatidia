...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Build the appropiate container for a given Dockerfile')
    ...
    arguments = ["docker", "build", "-t", f"ommatidia-py{versions['VERSION_PYTHON']}-cv{versions['VERSION_OPENCV']}-np{versions['VERSION_NUMPY']}:{version}"]
